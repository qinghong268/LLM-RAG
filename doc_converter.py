...
def convert_single_doc(doc_path, output_path, pythoncom, wc):
    """转换单个.doc文件为.docx，返回是否成功"""
    try:
        # 检查文件是否存在
...
def batch_convert_docs():
    ...
    print(f"\n🔍 正在扫描docs文件夹: {docs_folder}")
    
    # 查找所有.doc文件（统一使用小写扩展名）
    doc_files = []
    for file_path in docs_folder.iterdir():
        if file_path.is_file() and file_path.suffix.lower() == '.doc':
    ...
    for i, doc_file in enumerate(unique_files, 1):
        ...
        # 检查路径长度
        full_path = str(doc_file.absolute())
        if len(full_path) > 200:
            print(f"  ⚠️  警告: 文件路径过长 ({len(full_path)} 字符)，建议缩短文件名")
        
        # 检查输出文件是否已存在
        output_path = doc_file.with_suffix('.docx')
        if output_path.exists():
            print(f"  ⚠️  目标文件已存在，跳过: {output_path.name}")
            success_count += 1
            continue
        
        success = convert_single_doc(
            str(doc_file), 
            str(output_path), 
            pythoncom, 
            wc
        ...
        if success:
            ...
            try:
                ...
                try:
                    ...
                    print(f"  ⚠️  无法删除原始文件: {e}")
                
                success_count += 1
                
            except Exception as e:
                print(f"  ⚠️  转换成功但备份/清理失败: {e}")
                success_count += 1
        else:
    ...
    print(f"    - .doc文件: {len(remaining_doc_files)} 个")
    
    backup_files = list(backup_folder.glob("*.doc"))
    print(f"  📂 docs/backup文件夹中:")
...
def main():
    ...
    try:
        ...
        print("\n开始自动转换...")
        success = batch_convert_docs()
        end_time = time.time()
        
        print(f"\n⏱️  转换耗时: {end_time - start_time:.2f}秒")
